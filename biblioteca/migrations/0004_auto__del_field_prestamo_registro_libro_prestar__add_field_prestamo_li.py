...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Prestamo.registro_libro_prestar'
        db.delete_column('biblioteca_prestamo', 'registro_libro_prestar')

        # Adding field 'Prestamo.libro'
        db.add_column('biblioteca_prestamo', 'libro',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=1, to=orm['biblioteca.Libro']),
                      keep_default=False)

        # Deleting field 'Retorno.registro_libro_retorno'
        db.delete_column('biblioteca_retorno', 'registro_libro_retorno')

        # Adding field 'Retorno.libro'
        db.add_column('biblioteca_retorno', 'libro',
                      self.gf('django.db.models.fields.related.ForeignKey')(default=1, to=orm['biblioteca.Libro']),
                      keep_default=False)

    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'Prestamo.registro_libro_prestar'
        raise RuntimeError("Cannot reverse this migration. 'Prestamo.registro_libro_prestar' and its values cannot be restored.")
        # Deleting field 'Prestamo.libro'
        db.delete_column('biblioteca_prestamo', 'libro_id')


        # User chose to not deal with backwards NULL issues for 'Retorno.registro_libro_retorno'
        raise RuntimeError("Cannot reverse this migration. 'Retorno.registro_libro_retorno' and its values cannot be restored.")
        # Deleting field 'Retorno.libro'
        db.delete_column('biblioteca_retorno', 'libro_id')
